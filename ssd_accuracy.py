...
def main(args):
    ...
    failed_dir = os.path.expanduser(args.failed_dir)
                ...
                if not os.path.exists(output_filename):
                    try:
                        ...
                        if img.ndim<2:
                            ...
                            text_file.write('%s\n' % (output_filename))
                            copyfile(image_path, failed_filename)
                            continue
                        if img.ndim == 2:
                        ...
                        bounding_boxes = boxes[np.argwhere(scores>0.3).reshape(-1)]
                        ...
                        if nrof_faces>0:
                            ...
                            for i, det in enumerate(det_arr):
                                ...
                                cropped = img[det[0]:det[2], det[1]:det[3], :]
                                ...
                                text_file.write('%s %d %d %d %d\n' % (output_filename_n, det[0], det[1], det[2], det[3]))
                        else:
                            print('Unable to align "%s"' % image_path)
                            text_file.write('%s\n' % (output_filename))
                            copyfile(image_path, failed_filename)
...
def parse_arguments(argv):
    ...
    parser.add_argument('failed_dir', type=str, help='Directory with failed aligned')
